package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-feedreader/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-feedreader"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/feeds",s.handleListFeeds)
    s.mux.HandleFunc("POST /api/feeds",s.handleCreateFeed)
    s.mux.HandleFunc("DELETE /api/feeds/{id}",s.handleDeleteFeed)
    s.mux.HandleFunc("GET /api/feeds/{id}/items",s.handleListItems)
    s.mux.HandleFunc("POST /api/feeds/{id}/items",s.handleAddItem)
    s.mux.HandleFunc("POST /api/items/{id}/read",s.handleMarkRead)
